...
namespace System.Collections.Generic
{
  ...
  public class MyDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>, ISerializable, IDeserializationCallback where TKey : notnull
  {
        ...
        this(dictionary != null ? dictionary.Count : 0, comparer)
    {
      ...
      if (dictionary.GetType() == typeof(MyDictionary<TKey, TValue>))
      {
        MyDictionary<TKey, TValue> d = (MyDictionary<TKey, TValue>)dictionary;
        int count = d._count;
      ...
      else
      {
        do
        {
          ...
          if (entries[i].hashCode == hashCode && comparer.Equals(entries[i].key, key)
                                              && NonRandomizedStringEqualityComparer.Default.Equals(entries[i].key.ToString(), key.ToString())) 
              //Wenn es eine HashCoallision gibt, dann auf String Vergleich zurückfallen!
          {
            if (behavior == InsertionBehavior.OverwriteExisting)
            {
              entries[i].value = value;
    ...
    object? IDictionary.this[object key]
    {
      ...
      set
      {
        ...
        ThrowHelper.IfNullAndNullsAreIllegalThenThrow<TValue>(value);
